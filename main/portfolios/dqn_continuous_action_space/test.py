...
from .train import PortfolioVenv, portfolio_tickers, result_df
...
model = SAC.load(str(current/Path("cont_action_space_model")))
env = PortfolioVenv(test_df, init_cash=10_000, window_size=100, tickers=portfolio_tickers)
...
while not done:
    ...
    obs, reward, terminated, truncated, _ = env.step(action) # obs = array
    obss.append(obs)
    actions.append(action)

    prev_weights = weights_taken[-1] if weights_taken else 0
    weights_taken.append(action)
